...
const AddCover = ({setUploadedCover, replaceUrl}: AddCoverProps) => {
  ...
  const handleButton = () => imgInputRef.current?.click()
  ...
  return ( 
    <div>
      <input 
        ...
        className="hidden"
      />
      <button type="button" onClick={handleButton} className="flex items-center gap-2">
        <ImageIcon size={20} />
        <span>
          {!!replaceUrl ? "Change Cover" : "Add Cover"}
        </span>        
      </button>
      {isUploading && <p className="text-green-400">Uploading...</p>}
    </div>
   );
}
