...
const PasswordEmailForm = () => {
  const  [isPending, startTransition]  = useTransition();
    ...
    const {  register,  handleSubmit, formState: { errors } } = useForm<PasswordEmailSchemaType>({
  ...
  return (  
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col max-w-[500px] m-auto mt-8 gap-2" >
      ...
      <Button type="submit" label={isPending ? "Submitting ..." : "Send Reset Email"} outline disabled={isPending} />      
    </form>
  );
}
